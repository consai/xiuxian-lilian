...
5. **允许长期成长但避免数值失控**：百分比、减伤和控制均使用软上限。
...
| 属性 | 核心身份 | 主要影响 | 次要影响 |
|------|----------|----------|----------|
| 肉身 `roushen` | 生存与近战 | 气血、物攻、物防、负重 | 出手速度、气血恢复 |
| 灵力 `lingli` | 法术资源与威力 | 法力、法攻、法防 | 飞行续航、阵法驱动 |
| 神识 `shenshi` | 精准、反应与控制 | 出手速度、控制强度 | 法攻、法防、感知、制造 |
| 身法 `shenfa` | 位移与追逃 | 移动、追逃 | 先手修正 |

### 3.1 属性边界

- 灵力主要提供“量”和法术强度；控制强度由神识负责。
- 神识不提供大量法力，避免法修只堆神识。
...
| 控制流 | 神识 | 灵力 | 先手、控制 | 正面数值较低 |
| 游斗流 | 身法 | 神识 / 肉身 | 追逃、风筝 | 惧怕控制与范围攻击 |
...
```text
气血上限 hp_max = 肉身 × 10 + 80 + 固定加值
法力上限 mp_max = 灵力 × 5 + 80 + 固定加值
...
```text
物理攻击 physical_atk = 肉身 × 2.5 + 武器攻击 + 固定加值
法术攻击 magic_atk = 灵力 × 2.3 + 神识 × 0.5 + 法器攻击 + 固定加值

物理防御 physical_def = 肉身 × 1.2 + 护甲防御 + 固定加值
法术防御 magic_def = 灵力 × 1.1 + 神识 × 0.8 + 法器防御 + 固定加值
```
...
```text
出手速度 action_speed = 身法 × 2 + 神识 × 0.5 + 100 + 功法加值
...
物理伤害 = 伤害值 × 物攻 / (物攻 + 目标物防)
法术伤害 = 伤害值 × 法攻 / (法攻 + 目标法防)
实际控制时间 = 基础控制时间 × 控制强度 / (控制强度 + 目标控制抗性)
```
### 5.4 控制

```text
控制强度 control_power = 神识 × 1.6 + 灵力 × 0.5 + 功法加值
控制抗性 control_resist = 肉身 × 1 + 神识 × 1.2 + 功法加值
...
穿透先降低目标防御，易伤、伤害加成、护盾在攻防结算之后处理，并统一记录在战斗报告中。
...
境界差应通过境界基础值、技能阶位和攻防数值体现，不建议使用“一境界直接伤害翻倍”的硬倍率。这样低境界专精角色仍有越级可能，但代价明显。
...
控制、元素克制、制造成功率和福缘保底放在后续迭代，避免一次改动过大。
