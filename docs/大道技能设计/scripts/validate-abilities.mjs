...
const dao = JSON.parse(await readFile(new URL("../config/dao_tree.json", import.meta.url), "utf8"));
const config = JSON.parse(await readFile(new URL("../config/abilities.json", import.meta.url), "utf8"));
const catalog = JSON.parse(await readFile(new URL("../config/effect_catalog.json", import.meta.url), "utf8"));
const errors = [];
...
for (const realm of ["qi", "foundation", "core", "nascent", "spirit", "void", "integration", "mahayana", "tribulation"]) {
