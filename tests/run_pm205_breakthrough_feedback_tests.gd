...
func _run_all() -> void:
	...
	_expect_contains(TupoServiceScript.major_gap_hint(blocked), "还差 330", "reports missing total")
	_expect_contains(TupoServiceScript.major_gap_hint(blocked), "优先补", "reports main gaps")
