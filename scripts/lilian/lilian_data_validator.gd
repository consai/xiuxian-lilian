...
static func _validate_location(location: Dictionary, location_id: String) -> PackedStringArray:
	...
	for old_key in ["common_event_pool", "map_event_pool", "common_event_generation", "lilian_mode", "enemy_pools"]:
...
static func _validate_location_node_support(location: Dictionary, location_id: String) -> PackedStringArray:
	...
	for event_id_v in location.get("event_pool", []) as Array:
		...
		if LilianEventServiceScript.is_decision_event(event):
			supported_types[EnumLilianNodeTypeScript.ID_DECISION] = true
			continue
		if event_type not in ["travel", "gather", "recover", "hazard", "battle", "elite", "boss"]:
			...
			continue
		supported_types[EnumLilianNodeTypeScript.from_event(event)] = true
...
static func _validate_event(
	...
	if LilianRulesServiceScript.is_battle_type(str(event.get("type", ""))):
		...
		for msg in ZhandouInitData.collect_errors(_build_sample_battle_init(sample_event, game_state)):
...
static func _build_sample_battle_init(event: Dictionary, game_state: Node) -> Dictionary:
	...
	return {
		"player": player,
		"enemy": LilianEventServiceScript.build_battle_enemy(event),
		"enemies": LilianEventServiceScript.build_battle_enemies(event),
