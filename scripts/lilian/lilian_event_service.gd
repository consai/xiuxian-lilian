...
static func candidates_for_node(
	...
	for fallback_type in _node_type_fallbacks(type_id):
		...
		if not typed.is_empty():
			return typed
	return _candidates_matching_node_type(candidates, EnumLilianNodeTypeScript.ID_TRAVEL)
...
static func _battle_candidates(candidates: Array) -> Array:
	...
	for event_v in candidates:
		if not event_v is Dictionary:
			continue
		var event := event_v as Dictionary
		if LilianRulesServiceScript.is_battle_type(str(event.get("type", ""))):
			out.append(event)
	return out
...
static func _generated_battle_event_for_node(
	...
	if type_id == EnumLilianNodeTypeScript.ID_ELITE:
		event_type = "elite"
	elif type_id == EnumLilianNodeTypeScript.ID_BOSS:
		event_type = "boss"
	...
	return {
		...
		"risk_text": str(node.get("risk_text", EnumLilianNodeTypeScript.label(type_id))),
		"weight": 1,
...
static func _materialize_battle_event(
	...
	var type_id := str(node.get("type", EnumLilianNodeTypeScript.from_event(event)))
	var monster := _pick_location_monster_for_node(location, type_id, rng)
	if monster.is_empty():
		return event
	var monster_id := str(monster.get("id", "")).strip_edges()
...
static func resolve_decision_option(
	...
	if trigger_id != "":
		...
		return chained
	var rewards := LilianRewardServiceScript.roll_event_rewards(
		{
	...
	var loot := LilianLogServiceScript.format_rewards(rewards)
	if loot != "":
...
static func resolve_non_battle_event(
	...
	match event_type:
		"gather":
			...
			var gather_result := LilianLogServiceScript.gather_outcome(event, rewards)
			return {
				"ok": true,
				"rewards": rewards,
				"scene": scene,
		...
		"travel":
			var travel_result := LilianLogServiceScript.travel_outcome(event)
			return {
				"ok": true,
				"rewards": [],
				"scene": scene,
		...
		"recover", "hazard":
			...
			var hazard_result := LilianLogServiceScript.format_effect_lines(effect_lines)
			return {
				"ok": true,
				"rewards": [],
				"scene": scene,
...
static func _filter_candidates(
	...
	for event_v in event_pool_for_location(location, generated_events):
		...
		if str(event.get("type", "")).strip_edges() == "boss" and node_type != EnumLilianNodeTypeScript.ID_BOSS:
			continue
		out.append(event)
	return out
...
static func _event_matches_node_type(event: Dictionary, type_id: String) -> bool:
	if type_id == EnumLilianNodeTypeScript.ID_DECISION:
	...
	match type_id:
		EnumLilianNodeTypeScript.ID_GATHER:
			return event_type == "gather" and not bool(event.get("once_per_lilian", false))
		EnumLilianNodeTypeScript.ID_TREASURE:
			return event_type == "gather"
		EnumLilianNodeTypeScript.ID_RECOVER, EnumLilianNodeTypeScript.ID_REST:
