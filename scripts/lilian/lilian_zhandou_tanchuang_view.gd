...
func apply_event(event: Dictionary, _depth: int = 0) -> void:
	...
	var enemy := enemies[0] as Dictionary if not enemies.is_empty() else LilianEventServiceScript.build_battle_enemy(event)
	...
	var icon := ZhandouInitDataScript._resolve_icon_texture(enemy)
	if icon != null:
