...
func _ready() -> void:
	...
	if not LilianState.log_updated.is_connected(_refresh_log_display):
...
func _refresh_all() -> void:
	var location: Dictionary = LilianState.effective_location()
...
func _refresh_log_display() -> void:
	...
	for entry_v in LilianState.event_log:
		log_lines.append(LilianLogServiceScript.format_bbcode(entry_v as Dictionary))
	log_label.text = "\n\n".join(log_lines)
	_scroll_log_to_latest(log_label)


func _refresh_event_presentation() -> void:
	var is_decision: bool = not LilianState.pending_decision_event.is_empty()
	...
	if is_decision:
		...
		for index in card_nodes.size():
			...
			if index >= LilianState.current_choices.size():
				card.visible = false
				continue
			card.visible = true
			card.setup(LilianState.current_choices[index] as Dictionary)
...
func _is_pending_battle_dismissed() -> bool:
	if LilianState.phase != "battle" or LilianState.pending_battle_event_id == "":
		return false
	if _locked:
		return false
...
func _can_manual_exit() -> bool:
	return LilianState.can_exit() or _is_pending_battle_dismissed()


func _refresh_step_label() -> void:
	var step := %Step as Label
	var base := "过程第 %d 日 · %d 件事" % [LilianState.days, LilianState.steps]
...
func _can_open_utility_panels() -> bool:
	if not LilianState.active:
		return false
	if SceneManager.is_lilian_zhandou_overlay_active():
		return false
	return true
...
func _advance_auto_step() -> void:
	...
	_locked = true
	_refresh_controls()
	var began: Dictionary = LilianState.advance_day()
	_locked = false
...
func _complete_current_step() -> void:
	...
	_locked = true
	_refresh_controls()
	var result: Dictionary = LilianState.complete_current_step()
	_locked = false
...
func _on_event_chosen(event_id: String) -> void:
	...
	_locked = true
	_refresh_controls()
	var result: Dictionary = LilianState.choose_event(event_id)
	_locked = false
	if not bool(result.get("ok", false)):
		_refresh_all()
		return
...
func _route_switch_summary() -> Dictionary:
	var nodes_by_id := {}
	var current_layer := 0
	var current_node := LilianMapServiceScript.node_by_id(LilianState.map_nodes, LilianState.current_node_id)
	if not current_node.is_empty():
		current_layer = int(current_node.get("layer", 0))
	for node_v in LilianState.map_nodes:
		if not node_v is Dictionary:
			continue
		var node := node_v as Dictionary
		nodes_by_id[str(node.get("id", ""))] = node
	var outgoing_count_by_node := {}
	for edge_v in LilianState.map_edges:
...
func _resize_map_world() -> void:
	...
	var compact := LilianMapServiceScript.is_compact_map(LilianState.map_nodes)
	var target_width := (
		LilianMapServiceScript.COMPACT_MAP_WIDTH
		if compact
		else maxf(1500.0, viewport_size.x * 1.55)
	)
	var target_height := (
		LilianMapServiceScript.COMPACT_MAP_HEIGHT
		if compact
...
func _sync_loot_items() -> void:
	...
	var rewards: Array = LilianState.loot
...
func _on_map_node_selected(node_id: String) -> void:
	if _locked:
		return
	# 取消战前弹窗后，点当前遭遇节点重新打开
	if _is_pending_battle_dismissed() and node_id == LilianState.current_node_id:
		_reopen_pending_battle_popup()
		return
	_locked = true
	_refresh_controls()
	var began: Dictionary = LilianState.choose_map_node(node_id)
	_locked = false
	if not bool(began.get("ok", false)):
		var feedback := str(began.get("feedback", began.get("error", ""))).strip_edges()
		if feedback != "":
