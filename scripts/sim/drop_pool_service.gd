...
static func roll_event_rewards(event: Dictionary, rng: RandomNumberGenerator, context: Dictionary = {}) -> Array:
	...
	for result_v in event.get("results", []) as Array:
		...
		match str(result.get("type", "")):
			"drop":
				rewards.append_array(roll_pool_for_event(event, str(result.get("drop_pool", event.get("drop_pool", ""))), rng, int(result.get("rolls", 1)), context))
			"rewards":
				rewards.append_array(_roll_entries(result.get("rewards", []) as Array, rng, int(result.get("rolls", 1)), context))
	if rewards.is_empty() and str(event.get("drop_pool", "")).strip_edges() != "":
		rewards = roll_pool_for_event(event, str(event.get("drop_pool", "")), rng, int(event.get("reward_rolls", 1)), context)
