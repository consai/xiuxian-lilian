...
func _apply_result(result: Dictionary) -> void:
	var quality := str(result.get("quality", EnumLiandanQuality.LABEL_NONE))
...
func _product_quality_rank(product_id: String, recipe_id: String) -> int:
	if product_id == "items_WasteDan":
		return EnumLiandanQuality.rank(EnumLiandanQuality.LABEL_WASTE)
	var recipe := LiandanServiceScript.recipe_by_id(recipe_id)
	for quality_key_v in (recipe.get("products", {}) as Dictionary).keys():
		...
		if str((recipe.get("products", {}) as Dictionary).get(quality_key_v, "")) == product_id:
			return EnumLiandanQuality.rank(quality_key)
	return 0


func _apply_quality_style(quality: String) -> void:
	var color := EnumLiandanQuality.result_color(quality)
