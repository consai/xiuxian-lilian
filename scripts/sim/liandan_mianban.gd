...
@onready var _batch_popup: LiandanPiliangTanchuang = %BatchPopup
...
func _refresh_pill_preview(recipe: Dictionary) -> void:
	var product_id := LiandanServiceScript.recipe_preview_product_id(recipe)
	if product_id == "":
...
func _format_probabilities(probabilities: Dictionary) -> String:
	...
	for quality_label in EnumLiandanQuality.SUCCESS_LABELS:
		lines.append("%s  %5.1f%%" % [
			EnumLiandanQuality.display_name(quality_label),
...
func _start_brew(batch_count: int, preview: Dictionary) -> void:
	...
	var nav: Dictionary = SceneManager.go_liandan_jindu_quanping({
		"recipe_id": str(recipe.get("id", "")),
		"strategy_id": str(strategy.get("id", "")),
