...
static func _special_method_sources(method_slots: Dictionary) -> Array:
	...
	for slot_key in weights.keys():
		...
		if method_id != "":
			var bonus := CultivationMethodServiceScript.breakthrough_bonus(method_id)
			points = int(round(bonus * float(weights[slot_key])))
			var method_name := str(CultivationMethodServiceScript.by_id(method_id).get("name", method_id))
