class_name CombatVfxPresetLibrary
extends RefCounted
...
static func load_default() -> CombatVfxPresetLibrary:
	var lib := CombatVfxPresetLibrary.new()
	lib.load_from_index(JsonLoader.load_combat_vfx_index())
	return lib
...
func has_preset(preset_id: String) -> bool:
	var id := JsonLoader.normalize_combat_vfx_preset_id(preset_id)
	if id == "":
		return false
	if _sequence_cache.has(id):
		return true
	var path := JsonLoader.combat_vfx_preset_path(id)
	return path != "" and FileAccess.file_exists(path)


func get_preset_ids() -> Array:
	var names: Array = []
	var dir := DirAccess.open(JsonLoader.COMBAT_VFX_PRESETS_DIR)
	if dir == null:
		return names
	dir.list_dir_begin()
	var fn := dir.get_next()
...
func get_sequence(preset_ref: String) -> Array:
	var id := JsonLoader.normalize_combat_vfx_preset_id(preset_ref)
	if id == "":
	...
	if not has_preset(id):
		if id != _default_preset_id and has_preset(_default_preset_id):
			push_warning("CombatVfxPresetLibrary: 缺少 preset '%s'，回退 '%s'" % [id, _default_preset_id])
			return get_sequence(_default_preset_id)
		push_warning("CombatVfxPresetLibrary: 无法加载 preset '%s'" % id)
		return []
	var data := JsonLoader.load_combat_vfx_preset_file(id)
	var seq: Variant = data.get("sequence", [])
	if seq is Array:
		var steps := duplicate_steps(seq as Array)
		_sequence_cache[id] = steps
		return steps
	push_warning("CombatVfxPresetLibrary: preset '%s' 无 sequence" % id)
	return []
