...
const _DEFS := preload("res://scripts/fight/vfx/combat_vfx_step_defs.gd")


func play_sequence(ctx: CombatVfxContext, steps: Array) -> void:
	if ctx == null or ctx.host == null or steps.is_empty():
		return
	await _run_steps(ctx, steps)


func _run_steps(ctx: CombatVfxContext, steps: Array) -> void:
	for step_v in steps:
		if step_v is Dictionary:
			await _run_step(ctx, step_v as Dictionary)


func _run_step(ctx: CombatVfxContext, step: Dictionary) -> void:
	...
	match op:
		...
		_:
			push_warning("CombatActionExecutor: 未知 op '%s'" % op)


func _op_tween(ctx: CombatVfxContext, step: Dictionary) -> void:
	var role := str(step.get("actor", CombatVfxStepDefs.ACTOR_CASTER)).strip_edges()
	...
	if BattleDebugLog.enabled:
		BattleDebugLog.write("特效", "tween 开始", {
			"角色": role,
			"属性": prop,
			"锚点": anchor_name,
			"时长": duration,
	...
	if BattleDebugLog.enabled and is_instance_valid(actor):
		BattleDebugLog.write("特效", "tween 完成", {
			"角色": role,
			"属性": prop,
			"锚点": anchor_name,
...
func _op_tween_method(ctx: CombatVfxContext, step: Dictionary) -> void:
	var vfx := ctx.resolve_step_actor(step)
	if vfx == null:
		return
	var actor := vfx.get_actor()
...
func _op_parallel(ctx: CombatVfxContext, step: Dictionary) -> void:
...
func _branch_async(ctx: CombatVfxContext, branch_v: Variant, on_done: Callable) -> void:
...
func _op_impact(ctx: CombatVfxContext) -> void:
...
func _op_screen_shake(ctx: CombatVfxContext) -> void:
	...
	tw.set_trans(ctx.settings.crit_shake_trans).set_ease(ctx.settings.crit_shake_ease)
	var amp := ctx._f("crit.shake_amplitude", ctx.settings.crit_shake_amplitude)
	var freq := ctx._f("crit.shake_frequency", ctx.settings.crit_shake_frequency)
	var dur := ctx._f("crit.shake_duration", ctx.settings.crit_shake_duration)
	tw.tween_method(
		func(t: float) -> void:
...
func _await_tween(ctx: CombatVfxContext, tw: Tween) -> void:
...
static func _log_tween_skip(role: String, reason: String, step: Dictionary) -> void:
	var msg := "CombatActionExecutor: tween 跳过 (%s) actor=%s" % [reason, role]
	push_warning(msg)
	if BattleDebugLog.enabled:
		BattleDebugLog.write("特效", "tween 跳过", {
			"原因": reason,
			"角色": role,
