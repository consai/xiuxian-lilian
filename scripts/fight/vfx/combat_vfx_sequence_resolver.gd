class_name CombatVfxSequenceResolver
extends RefCounted
...
static func normalize_vfx_binding(vfx_v: Variant) -> Dictionary:
	if vfx_v is String:
		var id := JsonLoader.normalize_combat_vfx_preset_id(str(vfx_v))
		if id != "":
			return {"preset": id}
		return {}
	...
	if d.has("file"):
		d["preset"] = JsonLoader.normalize_combat_vfx_preset_id(str(d["file"]))
		d.erase("file")
	elif d.has("preset"):
		d["preset"] = JsonLoader.normalize_combat_vfx_preset_id(str(d["preset"]))
	return d
...
static func vfx_binding_from_skill_cfg(cfg: Dictionary) -> Dictionary:
	...
	for alias_key in ["vfx_file", "vfx_preset"]:
		if cfg.has(alias_key):
			var id := JsonLoader.normalize_combat_vfx_preset_id(str(cfg[alias_key]))
			if id != "":
				return {"preset": id}
	return {}


static func resolve(event: BattleVfxEvent, library: CombatVfxPresetLibrary) -> Array:
	if library == null:
		return []
	var vfx := {}
...
static func resolve_vfx_cfg(cfg: Dictionary, library: CombatVfxPresetLibrary) -> Array:
	if library == null:
		return []
	...
	if vfx_type == "":
		vfx_type = "melee"
	return library.get_sequence(CombatVfxPresetLibrary.legacy_preset_for_vfx_type(vfx_type))


static func overrides_from_event(event: BattleVfxEvent) -> Dictionary:
