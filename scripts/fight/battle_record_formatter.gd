class_name BattleRecordFormatter
extends RefCounted
...
func format_entry(entry: Dictionary, names: Dictionary = {}) -> String:
	...
	if kind == BattleRecordTypesScript.ACTION_BUFF_TICK:
		...
		if buff_label == "":
			buff_label = str(report.get(CombatReportScript.KEY_BUFF_NAME, "")).strip_edges()
		if buff_label == "":
			buff_label = "持续伤害"
	...
	var verb := "使用"
	if kind == BattleRecordTypesScript.ACTION_BASIC:
		verb = "普攻"
	elif kind == BattleRecordTypesScript.ACTION_ITEM:
		verb = "使用道具"
	elif kind == BattleRecordTypesScript.ACTION_EQUIP:
		verb = "催动法器"
	parts.append(verb)
	if action_name != "":
...
func format_summary(summary: Dictionary) -> String:
	...
	var title := "战斗结束"
	match outcome:
		BattleRecordTypesScript.OUTCOME_WIN:
			title = "胜利"
		BattleRecordTypesScript.OUTCOME_LOSS:
			title = "战败"
...
func _append_buff_names(parts: PackedStringArray, report: Dictionary) -> void:
	var buff_names_v: Variant = report.get(CombatReportScript.KEY_BUFF_NAMES, [])
