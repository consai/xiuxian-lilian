...
var _input := FightSceneInput.new()
var _presentation := FightScenePresentation.new()
var _combat := FightSceneCombatFlow.new()
...
func _ready() -> void:
	...
	_ctx.init_ok = FightSceneBootstrap.try_initialize(_ctx, _hud, editor_battle_init, editor_auto_sample, get_tree())
	if not _ctx.init_ok:
		set_process(false)
		return
...
func _bind_modules() -> void:
	var refs := FightSceneHudRefs.new()
