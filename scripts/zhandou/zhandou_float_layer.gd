...
func spawn(text: String, screen_pos: Vector2, tone: String, unit_key: String = "") -> void:
	...
	if not styles_v is Dictionary:
		ZhandouDebugLog.write("飘字", "ZhandouFloatLayer.styles 为空", {})
		return
	var style: Dictionary = (styles_v as Dictionary).get(tone, {}) as Dictionary
	if style.is_empty():
		push_warning("ZhandouFloatLayer: unknown tone '%s'" % tone)
		ZhandouDebugLog.write("飘字", "ZhandouFloatLayer 未找到 tone 样式", {
			"tone": tone,
			"text": t,
		})
		return
	if not _consume_spawn_budget(unit_key, tone):
		ZhandouDebugLog.write("飘字", "ZhandouFloatLayer spawn 被节流", {
			"unit": unit_key,
			"tone": tone,
		})
		return
	...
	inst.set_text(t)
	add_child(inst)
	ZhandouDebugLog.write("飘字", "ZhandouFloatLayer.spawn", {
		"unit": unit_key,
		"tone": tone,
		"text": t,
		"pos": screen_pos,
