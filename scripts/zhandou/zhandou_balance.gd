...
static func interval_cap_for(unit: ZhandouObj) -> float:
	if unit == null:
		return INTERVAL_BASE_SEC
	return interval_cap_from_spd(unit.get_attr(ZhandouObj.ATTR_SPD))
...
static func action_progress_rate_for(unit: ZhandouObj) -> float:
	if unit == null:
		return ACTION_PROGRESS_MAX / INTERVAL_BASE_SEC
	return action_progress_rate_from_spd(unit.get_attr(ZhandouObj.ATTR_SPD))
