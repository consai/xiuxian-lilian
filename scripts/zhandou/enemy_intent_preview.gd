...
static func _attacker_skill_cfg(attacker: ZhandouObj, skill_cfg: Dictionary, skill_id: int) -> Dictionary:
	...
	if slot.is_empty():
		return skill_cfg
	return ZhandouObj.merged_slot_runtime_cfg(slot, skill_cfg)
...
## 预览展示扣盾后的预计气血伤害，与 [method ZhandouObj.be_attacked] 一致。
...
static func _display_hp_damage(raw_damage: float, defender: ZhandouObj) -> int:
