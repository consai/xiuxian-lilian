class_name ZhandouVfxSettings
extends Resource

## 战斗表现全局可调参数（挂到 [ZhandouVfxManager] 或各 [ZhandouActorVfx] 上覆盖）。
...
@export_group("屏幕震屏")
## 屏幕/场景抖动幅度（像素）；由 VFX 预设 screen_shake 步骤触发。
@export_range(0.0, 24.0) var screen_shake_amplitude: float = 10.0
## 震屏总时长（秒）。
@export_range(0.05, 0.5) var screen_shake_duration: float = 0.22
## 震屏频率（越高越密）。
@export_range(10.0, 60.0) var screen_shake_frequency: float = 36.0
## 震屏缓动曲线。
@export var screen_shake_trans: Tween.TransitionType = Tween.TRANS_SINE
## 震屏缓动方式。
@export var screen_shake_ease: Tween.EaseType = Tween.EASE_OUT
