class_name ZhandouActorVfx
extends Node

## 挂在战斗单位 [Sprite2D] / [Node2D] 下：待机呼吸与静止姿势；攻击/受击由 [ZhandouActionExecutor] 驱动。
...
func _ready() -> void:
	...
	if _actor == null:
		push_error("ZhandouActorVfx: 父节点须为 Node2D（如 Sprite2D）")
		return
	rebaseline_rest_pose()


func apply_settings(new_settings: ZhandouVfxSettings) -> void:
	if new_settings == null:
		push_warning("ZhandouActorVfx.apply_settings: settings 为空")
		return
