...
static func find_skill_slot(enemy: ZhandouObj, skill_id: int) -> int:
...
static func can_use_skill(enemy: ZhandouObj, skill_id: int, skill_cfg: Dictionary) -> bool:
...
static func can_use_equip(enemy: ZhandouObj, slot_index: int, equip_cfg: Dictionary) -> bool:
	if enemy == null or slot_index < 0:
		return false
	...
	if float(slot.get("cd", 0.0)) > 0.0:
		return false
	var base_cfg := ZhandouObj._lookup_cfg(equip_cfg, equip_id)
