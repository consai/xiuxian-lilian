class_name ZhandouChangjingBootstrap
extends RefCounted
...
static func initialize_battle(ctx: ZhandouChangjingContext, hud: ZhandouChangjingHud, data: Dictionary) -> bool:
	return apply_battle_setup(ctx, hud, ZhandouInitData.resolve(data))


static func apply_battle_setup(ctx: ZhandouChangjingContext, hud: ZhandouChangjingHud, setup: ZhandouSetup) -> bool:
	if setup == null:
		push_error("ZhandouChangjing: ZhandouInitData.resolve 失败，无法开战")
		return false
	...
	ZhandouDebugLog.write("场景", "战斗初始化完成", {
		"玩家气血": ctx.battle_player.hp,
		"敌方气血": ctx.battle_enemy.hp,
		"玩家速度": ctx.battle_player.get_attr(ZhandouObj.ATTR_SPD),
		"玩家走条周期": ZhandouBalance.interval_cap_for(ctx.battle_player),
		"敌方速度": ctx.battle_enemy.get_attr(ZhandouObj.ATTR_SPD),
		"敌方走条周期": ZhandouBalance.interval_cap_for(ctx.battle_enemy),
		"时限": ctx.battle_time_limit,
	})
	return true


static func start_battle(ctx: ZhandouChangjingContext, hud: ZhandouChangjingHud) -> void:
	ctx.domain = ZhandouDomainService.new()
	ZhandouDebugLog.set_domain(ctx.domain)
	...
	if ctx.recorder != null:
		ctx.recorder.begin({
			...
			"player_name": str(ctx.record_names.get(ZhandouRecordTypes.UNIT_PLAYER, "")),
			"enemy_name": str(ctx.record_names.get(ZhandouRecordTypes.UNIT_ENEMY, "")),
		})
