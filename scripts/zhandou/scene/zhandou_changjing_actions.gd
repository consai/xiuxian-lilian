class_name ZhandouChangjingActions
extends RefCounted

## 槽位校验、出手解析与敌方 AI 决策。

const EnemyAiServiceScript = preload("res://scripts/zhandou/ai/enemy_ai_service.gd")
...
static func skill_id_at(actor: ZhandouObj, index: int) -> int:
...
static func can_actor_use_skill_at(
	...
	if cfg.is_empty():
		return false
	cfg = ZhandouObj.merged_slot_runtime_cfg(slot, cfg)
	return actor.can_pay_costs(cfg)


static func can_use_player_item_at(ctx: ZhandouChangjingContext, index: int) -> bool:
...
static func can_use_player_equip_at(ctx: ZhandouChangjingContext, index: int) -> bool:
...
static func get_skill_block_reason(ctx: ZhandouChangjingContext, index: int) -> Dictionary:
	...
	if cfg.is_empty():
		return empty_slot_reason()
	cfg = ZhandouObj.merged_slot_runtime_cfg(slot, cfg)
	var need := ZhandouObj.combat_resource_cost(cfg)
...
static func get_item_block_reason(ctx: ZhandouChangjingContext, index: int) -> Dictionary:
	if ctx.domain == null:
		return battle_busy_reason()
	if ctx.presentation_busy or not ctx.domain.can_player_act():
		return battle_busy_reason()
...
static func get_equip_block_reason(ctx: ZhandouChangjingContext, index: int) -> Dictionary:
	if ctx.domain == null:
		return battle_busy_reason()
	if ctx.presentation_busy or not ctx.domain.can_player_act():
		return battle_busy_reason()
	...
	var cfg := ZhandouObj._lookup_cfg(ctx.equip_cfg, equip_id)
	var cost_cfg := _slot_cost_cfg(slot, cfg)
	var need := ZhandouObj.combat_resource_cost(cost_cfg)
	var have := ctx.domain.player.mp
	if have < need:
		return build_insufficient_mp_reason(need, have)
	return ok_reason()
...
static func slot_view_for_type(ctx: ZhandouChangjingContext, slot_type: String, index: int) -> OneSkillView:
	match slot_type:
		"skill":
...
static func resolve_player_slot(ctx: ZhandouChangjingContext, index: int, skill_id: int) -> Dictionary:
	...
	if not bool(payload.get("ok", false)):
		ZhandouDebugLog.write("场景", "玩家出手失败", {
			"槽位": index,
			"技能ID": skill_id,
			"原因": ZhandouDebugLog.fail_reason_label(str(payload.get("reason", ""))),
		})
		return {}
	return payload
...
static func resolve_enemy_action_with_ai(ctx: ZhandouChangjingContext) -> Dictionary:
	if ctx.domain == null or ctx.battle_player == null:
		return {}
	...
	match action_type:
		...
		EnemyAiTypesScript.ACTION_SKILL:
			...
			desc = {"action_kind": ZhandouRecordTypes.ACTION_SKILL, "action_id": sid}
		...
		_:
			return {}
	if not bool(payload.get("ok", false)):
		ZhandouDebugLog.write("场景", "敌方 AI 出手失败", {
			"动作": action_type,
...
static func resolve_player_action_with_ai(ctx: ZhandouChangjingContext) -> Dictionary:
	if ctx.domain == null or ctx.battle_player == null:
		return {}
	ctx.battle_enemy = ctx.domain.enemy
	if ctx.battle_enemy == null:
...
static func _enemy_ai_cfg_for_current(ctx: ZhandouChangjingContext) -> Dictionary:
	if ctx.domain == null:
...
static func _enemy_ai_runtime_for_current(ctx: ZhandouChangjingContext) -> EnemyAiRuntimeState:
	if ctx.domain == null:
...
static func descriptor_for_item(payload: Dictionary) -> Dictionary:
	...
	if report_v is Dictionary:
		return {
			"action_kind": ZhandouRecordTypes.ACTION_ITEM,
			"action_id": int((report_v as Dictionary).get("item_id", -1)),
		}
	return {"action_kind": ZhandouRecordTypes.ACTION_ITEM, "action_id": -1}
...
static func descriptor_for_equip(payload: Dictionary) -> Dictionary:
	...
	if report_v is Dictionary:
		return {
			"action_kind": ZhandouRecordTypes.ACTION_EQUIP,
			"action_id": int((report_v as Dictionary).get("equip_id", -1)),
		}
	return {"action_kind": ZhandouRecordTypes.ACTION_EQUIP, "action_id": -1}


static func lookup_skill_cfg(ctx: ZhandouChangjingContext, skill_id: int) -> Dictionary:
...
static func emit_blocked_click_log(
	...
	ZhandouDebugLog.write("slot_click_blocked", "玩家点击不可用槽位", {
		"slot_type": slot_type,
		"index": index,
