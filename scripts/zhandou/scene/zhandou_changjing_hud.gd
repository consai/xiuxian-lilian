...
var _refs: ZhandouChangjingHudRefs
var _enemy_slot_nodes: Dictionary = {}
var _enemy_actor_nodes: Dictionary = {}


func bind(refs: ZhandouChangjingHudRefs) -> void:
	_refs = refs


func get_vfx() -> ZhandouVfxManager:
	return _refs.vfx if _refs != null else null


func get_float_layer() -> ZhandouFloatLayer:
...
func apply_battle(ctx: ZhandouChangjingContext, data: Dictionary) -> void:
...
func apply_combatant(ctx: ZhandouChangjingContext, side: String, row: Dictionary) -> void:
	if not row.has("hp") or not row.has("hp_max") or not row.has("mp") or not row.has("mp_max"):
		push_error("ZhandouChangjingHud.apply_combatant(%s): 缺少 hp/hp_max/mp/mp_max" % side)
		return
...
func sync_from_domain(ctx: ZhandouChangjingContext) -> void:
	if ctx.domain == null:
		return
	collect_enemy_formation_slots()
...
func sync_player_slot(ctx: ZhandouChangjingContext) -> void:
	...
	if row.is_empty():
		row = {
			"name": str(ctx.record_names.get(ZhandouChangjingContext.UNIT_PLAYER, "玩家")),
		}
	row["hp"] = unit.hp
...
func _apply_player_slot(ctx: ZhandouChangjingContext, row: Dictionary) -> void:
	...
	if _refs.vfx != null:
		_ensure_vfx_actor(ZhandouChangjingContext.UNIT_PLAYER, _slot_actor_node(slot))
...
func sync_enemy_formation(ctx: ZhandouChangjingContext) -> void:
...
func _apply_enemy_slot_node(ctx: ZhandouChangjingContext, node: Node2D, slot: Dictionary) -> void:
	...
	if bool(slot.get("current", false)):
		_enemy_actor_nodes[ZhandouChangjingContext.UNIT_ENEMY] = actor_node
		if _refs.vfx != null:
			_ensure_vfx_actor(ZhandouChangjingContext.UNIT_ENEMY, actor_node)


func _resolve_enemy_intent_row(ctx: ZhandouChangjingContext, enemy_index: int) -> Dictionary:
	var decision := ZhandouChangjingActions.preview_enemy_action(ctx, enemy_index)
	match str(decision.get("action_type", "")):
		...
		"item":
			...
			var item_cfg := ZhandouObj._lookup_cfg(ctx.item_cfg, item_id)
			return {
				"action_type": "item",
				"item_id": item_id,
				"icon": ZhandouInitData._resolve_icon_texture(item_cfg),
				"count": int(slot.get("count", 0)),
				"back_color": ZhandouInitData._quality_back_color(int(item_cfg.get("quality", 1))),
			}
		"equip":
			...
			var equip_cfg := ZhandouObj._lookup_cfg(ctx.equip_cfg, equip_id)
			return {
				"action_type": "equip",
				"equip_id": equip_id,
				"icon": ZhandouInitData._resolve_icon_texture(equip_cfg),
...
func _build_skill_intent_row(
	...
	var cfg := ZhandouObj._lookup_cfg(ctx.skill_cfg, skill_id)
	var row := {
		"action_type": action_type,
		"skill_id": skill_id,
		"icon": _resolve_skill_cfg_icon(ctx, skill_id),
		"back_color": ZhandouInitData._quality_back_color(int(cfg.get("quality", 1))),
	}
	...
	if unit == null:
		return row
	var preview_unit := ZhandouChangjingActions._enemy_unit_for_intent_preview(ctx, enemy_index, unit)
	return EnemyIntentPreview.enrich_skill_row(row, preview_unit, ctx.battle_player, cfg, skill_id)


func _resolve_skill_cfg_icon(ctx: ZhandouChangjingContext, skill_id: int) -> Texture2D:
	var tex := ZhandouInitData._resolve_icon_texture(ZhandouObj._lookup_cfg(ctx.skill_cfg, skill_id))
	if tex != null:
		return tex
	return ZhandouInitData._resolve_icon_texture({"icon": "ui_new/skill_03.png"})


func sync_enemy_header(ctx: ZhandouChangjingContext) -> void:
	...
	var avatar := ZhandouInitData._resolve_avatar_texture(row)
...
func sync_fight_time_label(ctx: ZhandouChangjingContext) -> void:
...
func sync_skill_cooldowns(ctx: ZhandouChangjingContext) -> void:
	if ctx.domain == null:
		return
	for i in ctx.skill_slots.size():
		var skill_id := ZhandouChangjingActions.skill_id_at(ctx.battle_player, i)
		if skill_id < 0:
		...
		var cfg := ZhandouChangjingActions.lookup_skill_cfg(ctx, skill_id)
	...
	for i in ctx.equip_slots.size():
		...
		var cfg := ZhandouObj._lookup_cfg(ctx.equip_cfg, equip_id)
		var total := float(equip_slot.get("cd_total", cfg.get("cd_total", cfg.get("cd", 0.0))))
		ctx.equip_slots[i].set_cooldown(cd, total)


func sync_item_and_equip_runtime_ui(ctx: ZhandouChangjingContext) -> void:
	if ctx.domain == null:
		return
	for i in ctx.item_slots.size():
		...
		var cfg := ZhandouObj._lookup_cfg(ctx.item_cfg, item_id)
...
func sync_runtime_slot_interactive(ctx: ZhandouChangjingContext) -> void:
	if ctx.domain == null:
		return
	for i in ctx.skill_slot_interactive.size():
		...
		ctx.skill_slot_interactive[i] = (
			skill_id >= 0
			and ZhandouChangjingActions.can_actor_use_skill_at(ctx, ctx.battle_player, i, skill_id)
		)
	for i in ctx.item_slot_interactive.size():
		ctx.item_slot_interactive[i] = ZhandouChangjingActions.can_use_player_item_at(ctx, i)
	for i in ctx.equip_slot_interactive.size():
		ctx.equip_slot_interactive[i] = ZhandouChangjingActions.can_use_player_equip_at(ctx, i)


func update_skill_input_enabled(ctx: ZhandouChangjingContext) -> void:
	var can_act := (
		ctx.domain != null
...
func can_attempt_escape(ctx: ZhandouChangjingContext) -> bool:
...
func escape_block_reason(ctx: ZhandouChangjingContext) -> String:
	if not bool(ctx.battle_flags.get("can_flee", true)):
		return "强敌封镇，无法逃脱"
...
func preview_escape_chance(ctx: ZhandouChangjingContext) -> float:
	if ctx.domain == null or ctx.domain.player == null:
		return 0.0
	return ZhandouBalance.escape_success_chance(
		ctx.domain.player.get_attr(ZhandouObj.ATTR_SPD),
...
func update_escape_button(ctx: ZhandouChangjingContext) -> void:
...
func spawn_unit_float(ctx: ZhandouChangjingContext, unit_id: String, text: String, tone: String = "skill") -> void:
...
func setup_battle_vfx(ctx: ZhandouChangjingContext) -> void:
	if _refs.vfx == null:
		push_error("ZhandouChangjing: 请在 zhandou_changjing.tscn 中放置 %ZhandouVfxManager 节点")
		return
...
func register_battle_actors(ctx: ZhandouChangjingContext) -> void:
	if _refs.vfx == null or ctx.scene == null:
		return
	if not is_instance_valid(_refs.sprite_left):
		push_warning("ZhandouChangjing: 玩家战斗精灵未就绪，延后注册 VFX 角色")
		ctx.scene.call_deferred("_deferred_register_vfx_actors")
		return
	_register_vfx_actor(ZhandouChangjingContext.UNIT_PLAYER, _slot_actor_node(_refs.sprite_left))
	var enemy_node := _actor_node_for_unit(ZhandouChangjingContext.UNIT_ENEMY)
	if is_instance_valid(enemy_node):
		_register_vfx_actor(ZhandouChangjingContext.UNIT_ENEMY, enemy_node)
	elif is_instance_valid(_refs.sprite_right):
		_register_vfx_actor(ZhandouChangjingContext.UNIT_ENEMY, _refs.sprite_right)
	_refs.vfx.refresh_all_actors()
	ZhandouDebugLog.write("场景", "VFX 角色注册完成", {
		"玩家": sprite_vfx_snapshot(ZhandouChangjingContext.UNIT_PLAYER),
		"敌方": sprite_vfx_snapshot(ZhandouChangjingContext.UNIT_ENEMY),
	})
...
func ensure_vfx_actors_for_combat() -> void:
	...
	if is_instance_valid(_refs.sprite_left):
		_ensure_vfx_actor(ZhandouChangjingContext.UNIT_PLAYER, _slot_actor_node(_refs.sprite_left))
	...
	var enemy_node := _actor_node_for_unit(ZhandouChangjingContext.UNIT_ENEMY)
	if is_instance_valid(enemy_node):
		_ensure_vfx_actor(ZhandouChangjingContext.UNIT_ENEMY, enemy_node)
	elif is_instance_valid(_refs.sprite_right):
		_ensure_vfx_actor(ZhandouChangjingContext.UNIT_ENEMY, _refs.sprite_right)
...
func unit_screen_pos(ctx: ZhandouChangjingContext, unit_id: String) -> Vector2:
...
func _actor_node_for_unit(unit_id: String) -> Node2D:
	if unit_id == ZhandouChangjingContext.UNIT_PLAYER:
	...
	if _enemy_actor_nodes.has(unit_id):
		...
		if is_instance_valid(node):
			return node
	if unit_id == ZhandouChangjingContext.UNIT_ENEMY and _enemy_actor_nodes.has(ZhandouChangjingContext.UNIT_ENEMY):
		var enemy_node := _enemy_actor_nodes[ZhandouChangjingContext.UNIT_ENEMY] as Node2D
		if is_instance_valid(enemy_node):
			return enemy_node
...
func _vfx_settings_for_actor(actor: Node2D) -> ZhandouVfxSettings:
	if _refs.vfx == null or _refs.vfx.settings == null or not is_instance_valid(actor):
		return null
	var settings := _refs.vfx.settings.duplicate() as ZhandouVfxSettings
	settings.actor_base_scale = maxf(0.01, maxf(absf(actor.scale.x), absf(actor.scale.y)))
	return settings


func sync_auto_battle_ui(ctx: ZhandouChangjingContext) -> void:
...
func render_battle_log_tail(ctx: ZhandouChangjingContext, entry: Dictionary) -> void:
...
func show_battle_result(ctx: ZhandouChangjingContext, summary: Dictionary) -> void:
