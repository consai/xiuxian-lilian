...
static var _domain: ZhandouDomainService = null


static func set_domain(domain: ZhandouDomainService) -> void:
	_domain = domain
...
static func format_advancing_time(domain: ZhandouDomainService = null) -> String:
...
static func log_unit(unit: ZhandouObj, role: String) -> Dictionary:
	if unit == null:
	...
	return {
		...
		"物理攻击": unit.get_attr(ZhandouObj.ATTR_PHYSICAL_ATK),
		"法术攻击": unit.get_attr(ZhandouObj.ATTR_MAGIC_ATK),
		"物理防御": unit.get_attr(ZhandouObj.ATTR_PHYSICAL_DEF),
		"法术防御": unit.get_attr(ZhandouObj.ATTR_MAGIC_DEF),
		"速度": unit.get_attr(ZhandouObj.ATTR_SPD),
		"行动进度速率": ZhandouBalance.action_progress_rate_for(unit),
		"预计出手间隔": ZhandouBalance.interval_cap_for(unit),
	}


static func log_domain(domain: ZhandouDomainService, tag: String = "快照") -> void:
	if not enabled or domain == null:
		return
	write("数据", tag, domain.get_debug_snapshot())


static func tick_progress(domain: ZhandouDomainService, delta: float) -> void:
