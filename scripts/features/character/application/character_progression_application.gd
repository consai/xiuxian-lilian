class_name CharacterProgressionApplication
extends RefCounted

const StateScript := preload("res://scripts/features/character/domain/character_progression_state.gd")
...
static func snapshot(savedata: Dictionary) -> Dictionary:
	...
	for key in StateScript.FIELDS:
		if not savedata.has(key):
			var message := "[character_progression_application:missing_state_slice] field=%s" % key
			push_error(message)
