class_name InventoryEquipCatalog
extends RefCounted

const PATH := "res://data/exportjson/zhuangbei_equips.json"
const EquipDefScript := preload("res://scripts/features/inventory/domain/equip_def.gd")
...
static func _parse_rows(raw: Variant) -> Array:
	...
	if not raw is Dictionary:
		push_error("InventoryEquipCatalog: config must be an object keyed by equip id")
		return out
	...
	for key_v in keys:
		...
		if not key.is_valid_int():
			push_error("InventoryEquipCatalog: key must be numeric id, got '%s'" % key)
			continue
		var equip_id := int(key)
		if equip_id <= 0:
			push_error("InventoryEquipCatalog: id must be positive, got %d" % equip_id)
			continue
		var row_v: Variant = rows[key_v]
		if not row_v is Dictionary:
			push_error("InventoryEquipCatalog: equip %d entry must be an object" % equip_id)
			continue
