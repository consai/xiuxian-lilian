class_name InventoryQuantitiesApplication
extends RefCounted

const StateScript := preload("res://scripts/features/inventory/domain/inventory_quantities_state.gd")
...
static func snapshot(savedata: Dictionary) -> Dictionary:
	...
	for key in StateScript.FIELDS:
		if not savedata.has(key):
			var message := "[inventory_quantities_application:missing_state_slice] field=%s" % key
			push_error(message)
...
static func commit(savedata: Dictionary, candidate: Variant) -> Dictionary:
	...
	for key in StateScript.FIELDS:
		savedata[key] = (value[key] as Dictionary).duplicate(true)
