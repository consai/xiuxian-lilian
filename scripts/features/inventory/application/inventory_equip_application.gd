class_name InventoryEquipApplication
extends RefCounted
...
static func snapshot(savedata: Dictionary) -> Dictionary:
	...
	for key in StateScript.FIELDS:
		if not savedata.has(key):
			var message := "[inventory_equip_application:missing_state_slice] field=%s" % key
			push_error(message)
...
static func commit(savedata: Dictionary, candidate: Variant) -> Dictionary:
	...
	for key in StateScript.FIELDS:
		savedata[key] = (value[key] as Array).duplicate(true)
